...
class MecanumDriveTest : public InternalFixture<::testing::TestWithParam<int>>
{
                                 ...
                                 const std::string &_odomTopic)
  {
                          ...
                          double yaw_component)
    {
      ...
      double desiredLinVelX = 10.5 * fb_component;
      double desiredLinVelY = 10.5 * lr_component;
      double desiredAngVel = 0.2 * yaw_component;
      ...
      // the mecanum drive has the odom frame at the center of the robot,
      // not the origin of the model. The model origin is offset from the
      // center of the robot so we need to account for that when calculating
      // the pose of the model from odometry.
      ...
      if (yaw_component != 0.0)
      {
        // the relative rotation has the same sign as the commanded rotation
        EXPECT_GT(desiredAngVel * relativeMotion.Yaw(), 0.0);
      }
    ...
    testCmdVel(0., 0., +1.); /* CW motion */
    testCmdVel(0., 0., -1.); /* CCW motion */
  }
};
...
TEST_P(MecanumDriveTest, IGN_UTILS_TEST_ENABLED_ONLY_ON_LINUX(PublishCmd))
{
  TestPublishCmd(
...
/////////////////////////////////////////////////
TEST_P(MecanumDriveTest, IGN_UTILS_TEST_DISABLED_ON_WIN32(OdomFrameId))
{
  // Start server
  ServerConfig serverConfig;
...
/////////////////////////////////////////////////
TEST_P(MecanumDriveTest, IGN_UTILS_TEST_DISABLED_ON_WIN32(OdomCustomFrameId))
{
  // Start server
  ServerConfig serverConfig;
  serverConfig.SetSdfFile(std::string(PROJECT_SOURCE_PATH) +
...
/////////////////////////////////////////////////
TEST_P(MecanumDriveTest, IGN_UTILS_TEST_DISABLED_ON_WIN32(Pose_VFrameId))
{
  // Start server
  ServerConfig serverConfig;
  serverConfig.SetSdfFile(std::string(PROJECT_SOURCE_PATH) +
...
/////////////////////////////////////////////////
TEST_P(MecanumDriveTest, IGN_UTILS_TEST_DISABLED_ON_WIN32(Pose_VCustomFrameId))
{
  // Start server
  ServerConfig serverConfig;
  serverConfig.SetSdfFile(std::string(PROJECT_SOURCE_PATH) +
...
/////////////////////////////////////////////////
TEST_P(MecanumDriveTest, IGN_UTILS_TEST_DISABLED_ON_WIN32(Pose_VCustomTfTopic))
{
  // Start server
  ServerConfig serverConfig;
  serverConfig.SetSdfFile(std::string(PROJECT_SOURCE_PATH) +
